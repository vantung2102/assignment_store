  ...
  def call
    ...
    cart.each do |item|
      ...
      if item['id_1'].present?
        ...
        if item['id_2'].nil?
          price = attribute.find_by(attribute_1: item['val_1']).price_attribute_product
        else
          price = attribute.where(attribute_1: item['val_1']).find_by(attribute_2: item['val_2']).price_attribute_product
        end
      end
      total = total + (price - product.discount)
    end
    ...
    if res_fee_charge['code'] == 200
      data = { fee: (res_fee_charge['data']['total'].to_f / 23000), total: total }
